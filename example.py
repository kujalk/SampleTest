...
def simulate_real_time_processing(csv_file_path, window_size=30, future_window=5, plot_interval=5):
    ...
    alpha_raise = 1.0  # Fast increase
    alpha_fall = 0.05  # Slow decrease
    ...
    for chunk in pd.read_csv(csv_file_path, chunksize=chunk_size, parse_dates=['timestamp']):
        ...
        # Calculate errors
        errors = np.abs(actuals_original - predictions_original)
        
        # Update threshold for each error
        for error in errors:
        ...
        all_real_values.extend(actuals_original)
        all_predicted_values.extend(predictions_original)
        all_anomalies.extend(anomalies)
        
        # Keep only the last 250 points for plotting
        max_length = 250
        if len(all_real_values) > max_length:
            all_real_values = all_real_values[-max_length:]
            all_predicted_values = all_predicted_values[-max_length:]
            all_anomalies = all_anomalies[-max_length:]
        
        # Check for alert condition: at least 3 anomalies in windows of 5
        alert_generated = False
        for i in range(0, len(all_anomalies) - 4):
            if np.sum(all_anomalies[i:i+5]) >= 3:
