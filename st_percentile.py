...
if 'params_set' not in st.session_state:
    ...
    st.session_state.percentile_threshold = 95
...
if 'sim_state' not in st.session_state:
    st.session_state.sim_state = {
        ...
        'model': None,
        'current_scaler': None,
        'error_history': []  # Store error history for percentile calculation
    }
...
def set_params():
    ...
    st.session_state.percentile_threshold = percentile_threshold
...
if not st.session_state.simulation_started:
    ...
    percentile_threshold = st.sidebar.slider("Percentile Threshold", min_value=90, max_value=99, value=st.session_state.percentile_threshold, step=1)
    ...
    st.sidebar.text(f"Percentile Threshold: {st.session_state.percentile_threshold}")
...
# Info about percentile threshold
st.sidebar.markdown("""
### Sensitivity Guide
- **Less Sensitive**: Higher Percentile (98-99)
- **Moderate**: Medium Percentile (95-97)
- **More Sensitive**: Lower Percentile (90-94)
""")
...
def initialize_simulation():
    ...
    if not st.session_state.sim_state['all_real_values']:
        ...
        st.session_state.sim_state['error_history'] = deque(maxlen=1000)  # Store longer history for percentile calculation
...
def simulate_real_time_anomaly_detection():
    ...
    with col2:
        st.subheader("Error Distribution and Threshold")
    ...
    try:
        ...
        while chunk_idx < total_chunks and not st.session_state.simulation_paused:
            ...
            for i in range(len(sequences)):
                ...
                if start_idx < len(sim_state['accumulated_data']):
                    actuals.append(sim_state['accumulated_data'].iloc[start_idx]['transactiontime'])
...
def update_plots(sim_state, timeseries_chart, threshold_chart):
    ...
    if time_points_list and real_values_list and predicted_values_list:
        ...
        # Plot the error histogram and threshold
        fig2, ax2 = plt.subplots(figsize=(10, 5))
        
        if errors_list and len(errors_list) > 5:  # Only if we have enough errors
            # Plot error histogram
            ax2.hist(errors_list, bins=20, alpha=0.6, color='skyblue')
            
            # Draw threshold line
            if thresholds_list:
                current_threshold = thresholds_list[-1]
                ax2.axvline(x=current_threshold, color='red', linestyle='--', 
                           label=f'Threshold ({st.session_state.percentile_threshold}th percentile)')
            
            ax2.set_title('Error Distribution and Threshold')
            ax2.set_xlabel('Error Value (Actual - Predicted)')
            ax2.set_ylabel('Frequency')
            ax2.legend()
        else:
            ax2.text(0.5, 0.5, 'Not enough data to display error distribution', 
                    horizontalalignment='center', verticalalignment='center', transform=ax2.transAxes)
        
        # Display the threshold histogram
